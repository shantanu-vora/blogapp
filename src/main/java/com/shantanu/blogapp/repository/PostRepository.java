...
//	@Query(value="select * from post where id in (select p.id from post p join post_tag pt on p.id = pt.post_id join tag t on t.id = pt.tag_id where lower(t.name) like %?1%)", nativeQuery = true)
	...
	@Query(value="select * from post where id in (select p.id from post p join post_tag pt on p.id = pt.post_id join tag t on t.id = pt.tag_id where t.id in :selectedTags)", nativeQuery = true)
	Page<Post> findByFilteredTags(Pageable pageable, @Param("selectedTags") List<Integer> selectedTags);
